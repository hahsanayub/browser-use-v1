...
3. **Schema Extraction**: Extracting data models, request bodies, response schemas, request headers, parameter definitions and the descriptions, Http Error Responses (including all 40X error and detailed response types and descriptions)
4. **Authentication Analysis**: Understanding authentication mechanisms, headers, API keys, OAuth flows, and security requirements
...
- Include a "pageType" field in the inal Extreacted Conent to indicates this in the final Extreacted Conent:
   - If the page content is an kind of overview, summary page, then for example set as:
        - "pageType": "This is a summary page that describes: xxxxx"
    - If the page content is the detailed page of an API document, then for example set as:
        - "pageType": "This is a detailed API documentation page of [API Endpoint Name] for the [API Service/API name] API"
- **DO NOT MODIFY OR SUMMARIZE**: Extract content exactly as it appears on the web page, do not summarize the description
- **DO NOT Fabricate Data**: If the page does not explicitly contain API endpoint paths, parameter names, types, or response status codes, do not guess or assume them.
	•	Only extract information that is clearly present in the page.
