export abstract class BaseTelemetryEvent {
	...
	properties(): Record<string, unknown> {
		const entries = Object.entries(this as Record<string, unknown>).filter(([key]) => key !== 'name');
...
export class AgentTelemetryEvent extends BaseTelemetryEvent implements AgentTelemetryPayload {
	name = 'agent_event';
	task: string;
	model: string;
	model_provider: string;
...
export class MCPClientTelemetryEvent extends BaseTelemetryEvent implements MCPClientTelemetryPayload {
...
export class MCPServerTelemetryEvent extends BaseTelemetryEvent implements MCPServerTelemetryPayload {
...
export class CLITelemetryEvent extends BaseTelemetryEvent implements CLITelemetryPayload {
	name = 'cli_event';
	version: string;
	action: string;
	mode: string;
