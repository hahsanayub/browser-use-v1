...
export class Registry<Context = unknown> {
		...
		time_execution_async('--execute_action')(
			async (
				...
				try {
					...
					throw new Error(`Error executing action ${action_name}: ${(error as Error).message}`);
				}
			},
		),
	);

	private replace_sensitive_data(params: any, sensitiveData: SensitiveDataMap, currentUrl: string | null) {
		...
		for (const [domainOrKey, content] of Object.entries(sensitiveData)) {
			if (content && typeof content === 'object' && !Array.isArray(content)) {
				if (currentUrl && !is_new_tab_page(currentUrl) && match_url_with_domain_pattern(currentUrl, domainOrKey)) {
	...
	private log_sensitive_data_usage(placeholders: Set<string>, currentUrl: string | null) {
