...
interface AgentWithState extends AgentReference {
	state: AgentReference['state'] & { model_dump?: () => Record<string, unknown> };
}

const getDeviceId = (agent: AgentReference) => agent.cloud_sync?.auth_client?.device_id ?? null;
...
export class CreateAgentStepEvent extends BaseEvent {
	...
	static fromAgentStep(
		...
		model_output: { current_state: { evaluation_previous_goal: string; memory: string; next_goal: string }; action: any[] },
