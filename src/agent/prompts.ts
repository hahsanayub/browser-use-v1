...
export class AgentMessagePrompt {
	...
	constructor(init: AgentMessagePromptInit) {
		...
		this.maxClickableElementsLength = init.max_clickable_elements_length ?? 40000;
...
`;
	...
	private agentStateDescription() {
		...
		const todoText = todoContents || '[Current todo.md is empty, fill it with your plan when applicable]';
		const timestamp = new Date().toISOString().slice(0, 16).replace('T', ' ');
		const stepInfo =
			this.stepInfo != null ? `Step ${this.stepInfo.step_number + 1} of ${this.stepInfo.max_steps}\n` : '';
...
`;
	...
	@observe_debug({ name: 'agent_message_prompt:get_user_message', ignore_input: true, ignore_output: true })
...
`;
		...
		if (use_vision && this.screenshots.length > 0) {
			const parts: Array<ContentPartTextParam | ContentPartImageParam> = [new ContentPartTextParam(stateDescription)];
			this.screenshots.forEach((shot, index) => {
				const label = index === this.screenshots.length - 1 ? 'Current screenshot:' : 'Previous screenshot:';
				parts.push(new ContentPartTextParam(label));
				parts.push(
					new ContentPartImageParam(
						new ImageURL(`data:image/png;base64,${shot}`, this.visionDetailLevel, 'image/png'),
					),
				);
			});
