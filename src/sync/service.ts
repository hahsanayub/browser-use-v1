...
export class CloudSync {
  ...
  private async sendEvent(event: BaseEvent) {
    try {
      ...
      if (status) {
        logger.debug(
          `Cloud sync HTTP ${status}: ${error?.response?.data ?? error}`
        );
    ...
    try {
      if (authClient.is_authenticated) {
        ...
        const divider = '─'.repeat(
          Math.max((process.stdout?.columns ?? 80) - 40, 20)
        );
        logger.info(divider);
  ...
  private async resendPendingEvents() {
    ...
    for (const event of events) {
      try {
        ...
        logger.warning(
          `Failed to resend event ${event.event_type}: ${(error as Error).message}`
        );
      }
    }
  }
