...
export class GmailService {
    ...
    constructor(options: {
        ...
        this.credentialsFile = options.credentials_file || path.join(this.configDir, 'gmail_credentials.json');
        this.tokenFile = options.token_file || path.join(this.configDir, 'gmail_token.json');
    }
    ...
    async authenticate(): Promise<boolean> {
        try {
            ...
            if (!this.creds || !this.creds.credentials || !this.creds.credentials.access_token) {
                if (this.creds && this.creds.credentials && this.creds.credentials.refresh_token) {
    ...
    async getRecentEmails(options: {
        ...
        try {
            ...
            // Get message list
            const results: GaxiosResponse<gmail_v1.Schema$ListMessagesResponse> = await this.service!.users.messages.list({
                userId: 'me',
            ...
            for (let i = 0; i < messages.length; i++) {
                ...
                const fullMessage: GaxiosResponse<gmail_v1.Schema$Message> = await this.service!.users.messages.get({
                    userId: 'me',
                    id: message.id!,
                    format: 'full',
                });
     ...
     * Send an email
     */
    async sendMessage(to: string, subject: string, body: string): Promise<gmail_v1.Schema$Message | null> {
        if (!this.isAuthenticated()) {
