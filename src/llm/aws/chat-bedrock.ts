import { BedrockRuntimeClient, ConverseCommand, type Tool as BedrockTool, type ToolChoice } from '@aws-sdk/client-bedrock-runtime';
...
export class ChatBedrockConverse implements BaseChatModel {
    ...
    constructor(model: string = 'anthropic.claude-3-5-sonnet-20240620-v1:0', region: string = process.env.AWS_REGION || 'us-east-1') {
        this.model = model;
    ...
    async ainvoke<T>(
        ...
        const systemMessage = messages.find((msg) => msg instanceof SystemMessage) as SystemMessage | undefined;
        ...
        if (output_format && 'schema' in output_format && output_format.schema) {
            try {
                const jsonSchema = zodToJsonSchema(output_format as unknown as z.ZodType, {
                    name: 'Response',
                    target: 'jsonSchema7',
                });
