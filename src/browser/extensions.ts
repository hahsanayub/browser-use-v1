 ...
 */
async function unpackCrx(crxPath: string, unpackedPath: string): Promise<boolean> {
	try {
 ...
 */
async function installExtension(extension: BrowserExtensionDescriptor): Promise<boolean> {
...
export async function isTargetExtension(target: any): Promise<{
	...
	try {
		...
		target_url = (await target.url?.()) || (target_ctx ? await target_ctx.url() : null);
	} catch (error: any) {
...
export async function loadExtensionFromTarget(
	...
	const { target_is_bg, target_is_extension, target_type, target_ctx, target_url, extension_id, manifest_version } =
		extensionInfo;
	...
	const existing_extension = extensions.find((ext) => ext.id === extension_id) || ({} as BrowserExtensionDescriptor);
...
export async function installExtensionsFromConfig(
	...
	try {
		...
		logger.error(`[❌] Extension installation failed: ${(error as Error).message}`);
	}
