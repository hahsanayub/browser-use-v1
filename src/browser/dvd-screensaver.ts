...
export class DVDScreensaver {
	...
	private colors = ['\x1b[31m', '\x1b[32m', '\x1b[33m', '\x1b[34m', '\x1b[35m', '\x1b[36m'];
	...
	private render(): void {
		...
		for (let row = 0; row < this.height; row++) {
			...
			for (let col = 0; col < this.width; col++) {
				...
				} else if (row === 0 || row === this.height - 1 || col === 0 || col === this.width - 1) {
					// Draw border
					line += '·';
				} else {
					line += ' ';
 ...
 * stopAnimation();
 */
export function showDVDScreensaver(message: string = 'Loading...', fps: number = 10): () => void {
 ...
 */
export function showSpinner(message: string = 'Loading...', fps: number = 10): () => void {
